...
public class GunLineRenderer : MonoBehaviour
{
    ...
    [SerializeField] private PhysicsGun _physicsGun;
    ...
    private void Start()
    {
        ...
        if (_physicsGun == null)
        {
            Debug.LogError($"{nameof(GunLineRenderer)} is missng a {nameof(PhysicsGun)}", this);
            return;
        }
    ...
    private void Update()
    {
        ...
        if (_physicsGun._grabbedRigidbody != null)
        {
            OnObjectGrabbed(_physicsGun._grabbedRigidbody.gameObject);
        }
        else
            StopLineRenderer();
    }
